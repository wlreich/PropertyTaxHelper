...
export function MarketAdjustmentPanel({data,propertyId,neighborhood=false}:{data:MarketAdjustment|null;propertyId:string;neighborhood?:boolean}) {
  ...
  return <section className="market-adjustment-panel" id="market-adjustment" aria-labelledby="market-adjustment-heading">
    ...
    <details><summary>How this estimate works and what is covered</summary>
      <p>We compare the first eligible dated preliminary record for this tax year with the same model using the previous year’s multiplier. All other current-year inputs stay fixed. The estimate is the difference between those two rounded model values. We include a home only when its component values and current multiplier reproduce its recorded preliminary improvement value within $1 and one residential building is verified.</p>
      ...
      {home?.preliminary_date&&<p>Your preliminary record: {home.preliminary_date}{home.prior_preliminary_date?`; prior preliminary: ${home.prior_preliminary_date}`:''}.</p>}
