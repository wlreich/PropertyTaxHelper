...
export function AdjustedComparisons({ subject, selected, release }: {
  ...
  const results = selected.map(property => propertyAdjustments(subject, property));
  ...
  return <section className="comparison-card comparison-adjusted" aria-labelledby="adjusted-comparison-heading">
    ...
    <dl className="comparison-summary comparison-adjusted-summary">
      ...
      <div><dt>Estimated adjusted median</dt><dd>{adjusted.median === null ? "Not available" : currency(adjusted.median)}</dd><dd className="comparison-summary-percent">{adjusted.count} complete estimates</dd></div>
    ...
    <p className="comparison-adjusted-note" role="status">{selected.length === 0 ? "Select comparison properties in Reported values to see their adjustment breakdowns." : `${adjusted.count} of ${results.length} selected properties have complete adjustments. ${adjusted.excluded ? `${adjusted.excluded} incomplete calculations are excluded from the adjusted median.` : "Both medians use the same properties."}`} Your property is excluded from both medians.{reported.missing > 0 ? ` ${reported.missing} missing reported values are also excluded from the reported median.` : ""}</p>
    {adjusted.count > 0 && adjusted.excluded > 0 && <p className="comparison-adjusted-note">The medians use different sets. For the {adjusted.count} complete estimates only, the reported median is {currency(adjusted.pairedReportedMedian)}.</p>}
    ...
    {selected.length > 0 && <>
      ...
      <p className="comparison-adjusted-note">Only the land adjustment can currently be calculated. Open a property’s breakdown to see the inputs still needed.</p>
          ...
          {results.map(result => <Fragment key={result.property.property_id}>
            ...
            <tr id={`adjustment-${result.property.property_id}`} hidden={expanded !== result.property.property_id}><td colSpan={4} className="comparison-breakdown-cell">
              <div className="comparison-breakdown"><h5>Adjustment breakdown · {result.property.address}</h5>
                <table><caption className="comparison-sr-only">Adjustments for {result.property.address}</caption><thead><tr><th scope="col">Factor</th><th scope="col">Explanation and inputs</th><th scope="col">Adjustment</th></tr></thead><tbody>{result.lines.map(line => <tr key={line.factor}><th scope="row">{line.factor}</th><td>{line.explanation}{line.inputs.length > 0 && <dl className="comparison-calculation-inputs">{line.inputs.map(input => <div key={input.label}><dt>{input.label}</dt><dd>{currency(input.value)}</dd></div>)}</dl>}</td><td>{line.amount === null ? "Inputs needed" : signed(line.amount)}</td></tr>)}</tbody></table>
                <div className="comparison-partial-result"><div><strong>Land-adjusted subtotal</strong><p className="comparison-money">{result.landSubtotal === null ? "Not available" : `${currency(result.property.market_value)} ${result.lines[0].amount! < 0 ? "−" : "+"} ${currency(Math.abs(result.lines[0].amount!))} = ${currency(result.landSubtotal)}`}</p></div><p>This is a partial calculation. It is excluded from the adjusted median until every required adjustment can be calculated.</p></div>
    ...
    <p className="comparison-adjusted-note">Source: TCAD {release.tax_year} {release.roll_stage} records · Exported {release.export_date ?? "date not reported"}. Calculations use the documented 2026 methodology{release.tax_year !== 2026 ? `; applicability to ${release.tax_year} has not been verified` : ""}. Estimates are based on available inputs; TCAD may reach a different result. These are property values, not tax savings.</p>
  </section>;
}
