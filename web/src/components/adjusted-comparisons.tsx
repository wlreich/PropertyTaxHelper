        ...
        <table className="comparison-adjustment-table"><thead><tr><th scope="col">Property</th><th scope="col">Reported value</th><th scope="col">Total adjustment</th><th scope="col">ParcelSavvy estimate</th></tr></thead><tbody>
          {results.map(result => <Fragment key={result.property.property_id}>
            ...
            <tr id={`adjustment-${result.property.property_id}`} hidden={expanded !== result.property.property_id}><td colSpan={4} className="comparison-breakdown-cell">
              <div className="comparison-breakdown">{result.reviewReason && <p className="comparison-inline-note">{result.reviewReason}</p>}<h5>Adjustment breakdown · {result.property.address}</h5>
