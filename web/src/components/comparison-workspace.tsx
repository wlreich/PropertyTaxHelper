...
function ComparisonWorkspaceContent({data,initialIds,initialView="reported",initialStep="select",evidence,focusTarget,releaseNotice=null,inspection,setInspection}:WorkspaceProps & {inspection:ComparisonInspection|null;setInspection:(value:ComparisonInspection|null)=>void}) {
  ...
  return <>
    <div className="comparison-title"><div><h2 ref={heading} tabIndex={-1}>{initialStep==="select"?"Choose homes to compare":"Compare similar homes"}</h2><p>{initialStep==="select"?"Compare your home’s market value with homes like yours. If a similar home has a lower value, take a closer look at the differences. That can help you decide whether to protest and which properties may be useful as evidence.":`${selected.length} selected ${selected.length===1?"property":"properties"} compared with your home.`}</p></div>
    ...
    {initialStep==="results"&&<div className="comparison-view-controls"><div className="comparison-view-options"><span className="comparison-view-label">View values as</span><div className="comparison-view-switch" role="group" aria-label="Comparison values" aria-describedby="comparison-view-description"><button aria-pressed={view==="reported"} onClick={()=>changeView("reported")}>Reported values</button><button aria-pressed={view==="adjusted"} onClick={()=>changeView("adjusted")}>Estimated adjusted values</button></div><p id="comparison-view-description" className="comparison-view-description"><strong>Reported values</strong> are the Appraisal District’s market values for these homes. <strong>Estimated adjusted values</strong> use adjustment logic the district provided through open records to account for recorded differences between each home and yours. ParcelSavvy calculates these estimates; they are not the district’s official results.</p></div><div className="comparison-result-actions"><button ref={editButton} className="comparison-add-link" onClick={editSelection}>Edit selection ({active.length})</button></div></div>}
    {message&&<p role="status" className="comparison-inline-note">{message}</p>}
    {initialStep==="select"&&<div className="comparison-guidance"><p>Start with homes similar in location, size, age, and construction. Condition and other details matter too.</p><PropertySectionLink target="comparison-rules-heading">How matching works</PropertySectionLink></div>}
    {initialStep==="select"&&<>
    ...
    <div className="comparison-workspace">
      <section className="comparison-card comparison-suggestions" aria-labelledby="suggestions-heading">
        ...
        <p className="comparison-inline-note">{selectedTier?`Same recorded market area and class · living area within ${selectedTier.area}% · built within ${selectedTier.years} years.`:"All available tiers, ordered by closest recorded match unless sorted by value."} Each property appears in its closest qualifying tier.</p>
      ...
      <aside className="comparison-selection" aria-labelledby="selection-heading"><div className="comparison-card"><h3 id="selection-heading">Your comparison set</h3><p>{selected.length} of 10 properties selected</p>
        ...
        <p className="comparison-small">Your draft does not replace the active comparison until you apply it. Cancel keeps your active comparison. Applied selections are saved in the results link. Changing the assessment release resets unsaved edits.</p>
    ...
    {initialStep==="results"&&<section className="comparison-card comparison-results"><h3>Build evidence around meaningful differences</h3><p>Review recorded size, age, land and additional structures before deciding which differences matter.</p><Link className="comparison-text-button" href={`/protest-guide?property=${data.subject.property_id}`}>Review protest options →</Link></section>}
    <details className="comparison-card comparison-method"><summary id="comparison-rules-heading">How matching works</summary><p>ParcelSavvy starts with the Appraisal District’s 2026 Sale and Equity Grids methodology. Suggestions require the same recorded market area and construction class, then use progressively wider living-area and age ranges.</p>
      <p>Match tiers compare recorded market area, construction class, living area, and age. They are a starting point, not the Appraisal District’s complete comparable-property score. Condition and other eligibility factors remain unverified.</p>
      {data.release.tax_year!==comparisonMethod.year&&<p>{matchingCaveat}</p>}
      <div className="comparison-table-scroll" role="region" aria-label="Appraisal District equity tier criteria" tabIndex={0}><table><thead><tr><th scope="col">Tier</th><th scope="col">Living area</th><th scope="col">Condition</th><th scope="col">Class</th><th scope="col">Year built</th></tr></thead><tbody>{comparisonMethod.tiers.map(t=><tr key={t.tier}><th scope="row">{t.tier}</th><td>Within {t.area}%</td><td>{t.condition?`Within ${t.condition} ${t.condition===1?"step":"steps"}`:"Same"}</td><td>{t.classSteps?`Within ${t.classSteps} ${t.classSteps===1?"step":"steps"}`:"Same"}</td><td>Within {t.years} years</td></tr>)}</tbody></table></div>
      <p><strong>Limits of the match:</strong> ParcelSavvy can verify the recorded market area, construction class, living area, and age shown on this page, but not condition or every eligibility factor in the Appraisal District’s method. Homes with multiple living-area buildings are omitted when the primary building cannot be established. Suggestions are ordered by tier, size difference, age difference, and property ID; they are not the Appraisal District’s confirmed comparable list.</p>
      <p>The page labels the 2026 matching rules when you view another year. A property outside these suggestions may still be useful, and this page does not determine what an appraiser or hearing panel will accept. <Link href="/methodology">Read Data &amp; methodology</Link> for the source and coverage approach.</p>
    </details>
    <details className="comparison-card comparison-method"><summary>How adjusted values work</summary><p>ParcelSavvy starts with each selected property’s reported market value and applies supported adjustments for land, construction class, depreciation, living area, non-living details, additional buildings and features, and neighborhood differences.</p><p>The Appraisal District estimates rebuilding cost before depreciation, then reduces it for age and condition. Percent good is the percentage of estimated rebuilding cost remaining after depreciation. Land is valued separately. The district&apos;s final recorded value of the home and other features includes the neighborhood factor; it is not the rebuilding-cost estimate.</p><p>Calculations use the selected year’s reported inputs and published schedules. Estimates are withheld when required inputs or a supported annual schedule are unavailable, and only complete estimates enter the adjusted median. Open a property breakdown to review every input, signed amount, assumption, formula, and source.</p><p>Some inputs require documented approximations. For parcels with multiple improvements, the highest-reported-value improvement supplies living area, class, and depreciation inputs; non-living details and other buildings are adjusted separately. When percent good is unavailable, ParcelSavvy uses the Appraisal District’s published class-and-age schedule for the selected year and assumes average condition. Main-area rebuilding costs before depreciation may be reconstructed from rebuilding costs after depreciation, and neighborhood multipliers may be recovered from recorded non-land values. Each adjustment is truncated to whole dollars after unit rates are rounded to cents. Actual condition and other unreported differences may change the Appraisal District’s result.</p></details>
  </>;
}
