...
export function EvidenceWindowControls({window,error}:{window:EvidenceWindow;error?:string|null}){
 ...
 function submit(e:FormEvent){e.preventDefault();const next={targetYear:Number(targetYear),start,end},problem=evidenceWindowError(next);setMessage(problem??'');if(problem)return;
  ...
  transition(()=>router.push(`${path}?${params}${path.endsWith('/neighborhood')?'#recent-activity':''}`,{scroll:false}));
 }
 return <form className="evidence-window" onSubmit={submit} aria-label="Evidence research window" aria-busy={pending}>
  ...
  <label>Evidence start<input name="evidenceStart" type="date" required value={start} onChange={e=>setStart(e.target.value)}/></label><label>Evidence end<input name="evidenceEnd" type="date" required value={end} onChange={e=>setEnd(e.target.value)}/></label><button className="action-button" disabled={pending}>Apply dates</button></div>
