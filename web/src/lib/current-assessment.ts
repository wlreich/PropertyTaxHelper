...
export function currentAssessmentStory(current: Snapshot, snapshots: Snapshot[], evidence: ProtestObservation[], season: SeasonContext | null, unavailable = false) {
  ...
  const protest = recorded ? 'Protest recorded' : proposed && proposed.dollars < 0 ? 'Reduction evidence suggests a possible protest; no protest record found' : unavailable ? 'Protest records temporarily unavailable' : 'No protest found in available records';
