...
function model(property:ComparisonProperty, taxYear:number) {
  ...
  const inputs:TcadInputs={market:amount(property.market_value)?property.market_value:null,land:amount(property.land_value)?property.land_value:null,area:main?.main_area??null,
    ...
    secondary,mass};
  const secondaryInventory=costs?.improvements.filter(b=>b.id!==main?.id)??[];
  return {inputs,good,main,complete,secondaryInventory};
}
export function propertyAdjustments(subject:ComparisonProperty,comparable:ComparisonProperty,taxYear=2026) {
  ...
  const lines:AdjustmentLine[]=[
    {factor:"Land",amount:calculation.amounts.Land,explanation:"Your market land value minus the comparable’s market land value, including agricultural land at market value when reported. Agricultural use values are not used.",inputs:[input("Your land value",subject.land_value),input("Comparable land value",comparable.land_value)]},
    {factor:"Living area",amount:calculation.amounts["Living area"],explanation:"Difference in the primary buildings’ living areas × your rebuilding cost per sq ft. This before-depreciation rate is reconstructed from the main-area rebuilding cost after depreciation and estimated percent good; the main-area factor is 100%.",inputs:[input("Your living area (sq ft)",si.area,"number"),input("Comparable living area (sq ft)",ci.area,"number"),input("Your rebuilding cost per sq ft",calculation.subjectRate===null?null:`$${calculation.subjectRate.toFixed(2)}`)]},
    {factor:"Construction class",amount:calculation.amounts["Construction class"],explanation:si.classCode&&si.classCode===ci.classCode?"Same reported class; $0 follows the Appraisal District’s worked same-class comparisons. Different building configurations may need individual review.":"(Your rebuilding-cost rate ÷ comparable rate − 1) × the comparable’s estimated main-area rebuilding cost before depreciation. Reconstructed rates use estimated percent good.",inputs:[input("Your class",si.classCode),input("Comparable class",ci.classCode),input("Comparable rebuilding cost per sq ft",calculation.compRate===null?null:`$${calculation.compRate.toFixed(2)}`),input("Comparable rebuilding cost before depreciation",ci.mainRcn)]},
    {factor:"Percent good",amount:calculation.amounts["Percent good"],explanation:"Difference in percent good ÷ 100 × the comparable’s estimated main-area rebuilding cost after depreciation. Percent good is the percentage of estimated rebuilding cost remaining after depreciation.",inputs:[input("Your percent good",si.percentGood===null?null:`${si.percentGood}%`),input("Comparable percent good",ci.percentGood===null?null:`${ci.percentGood}%`),input("Your depreciation year",s.good?.year,"year"),input("Comparable depreciation year",c.good?.year,"year"),input("Comparable rebuilding cost after depreciation",ci.mainRcnld),input("Your estimate basis",s.good?.basis),input("Comparable estimate basis",c.good?.basis)]},
    {factor:"Non-living details",amount:calculation.amounts["Non-living details"],explanation:"Your primary building’s estimated rebuilding costs for non-living details, adjusted for age and condition, minus the comparable’s. These amounts are before neighborhood factors. Garages, porches, pools and other features attached to that building are counted here once.",inputs:[input("Your non-living rebuilding cost after depreciation",si.nonliving),input("Comparable non-living rebuilding cost after depreciation",ci.nonliving),input("Your recorded non-living features",s.main?.complete ? s.main.features.filter(f=>!["1ST","2ND","3RD"].includes(f.code)).map(f=>f.description||f.code).join(", ")||"None recorded" : null),input("Comparable recorded non-living features",c.main?.complete ? c.main.features.filter(f=>!["1ST","2ND","3RD"].includes(f.code)).map(f=>f.description||f.code).join(", ")||"None recorded" : null)]},
    {factor:"Additional improvements",amount:calculation.amounts["Additional improvements"],explanation:"Your recorded value of additional buildings and features minus the comparable’s. The Appraisal District groups each additional structure with its attached features, such as garages, porches and pools. These recorded non-land values already include neighborhood multipliers; they are not rebuilding-cost estimates and are counted once here. $0 means no additional value is recorded in this release.",inputs:[input("Your additional buildings & features",si.secondary),input("Comparable additional buildings & features",ci.secondary),
      ...s.secondaryInventory.flatMap(b=>[input(`Your additional record ${b.id} — reported value`,b.reported_value),input(`Your additional record ${b.id} — living area (sq ft)`,b.main_area,"number")]),
      ...c.secondaryInventory.flatMap(b=>[input(`Comparable additional record ${b.id} — reported value`,b.reported_value),input(`Comparable additional record ${b.id} — living area (sq ft)`,b.main_area,"number")]) ]},
    {factor:"Neighborhood",amount:calculation.amounts.Neighborhood,explanation:assumedEqualMass?"Same reported market area; equal neighborhood factors are assumed because a multiplier could not be recovered.":"(Your multiplier − comparable multiplier) ÷ comparable multiplier × comparable non-land market value. Multipliers are recovered from the recorded value of the home and other features divided by summed rebuilding costs after depreciation; equal factors give $0.",inputs:[input("Your market area",subject.neighborhood),input("Comparable market area",comparable.neighborhood),input("Your multiplier",si.mass===null?null:si.mass.toFixed(4)),input("Comparable multiplier",ci.mass===null?null:ci.mass.toFixed(4))]},
  ];
