...
function model(property:ComparisonProperty, taxYear:number) {
  ...
  const groupingReview=secondaryInventory.some(b=>b.reported_value===null || b.reported_value>0 || b.floors>0);
  return {inputs,good,main,complete,groupingReview,secondaryInventory};
}
export function propertyAdjustments(subject:ComparisonProperty,comparable:ComparisonProperty,taxYear=2026) {
  ...
  const reviewReason=s.groupingReview||c.groupingReview
    ? "Adjustment withheld: one or both properties have additional improvement records. How their living areas and features should be adjusted together needs review. Reported values remain available; this comparison is excluded from the adjusted median."
    : null;
  ...
  const totals=compatible&&!reviewReason?totalAdjustments(comparable.market_value,lines):{total:null,adjustedValue:null};
  const subtotal=amount(comparable.market_value)?money(comparable.market_value+lines.reduce((sum,l)=>sum+(l.amount??0),0)):null;
  return {property:comparable,lines,...totals,reviewReason,partialSubtotal:!reviewReason&&subtotal!==null&&subtotal>=0?subtotal:null,
