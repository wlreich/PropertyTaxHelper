...
function model(property:ComparisonProperty, taxYear:number) {
  ...
  const complete=!!main?.complete && costs!.improvements.every(b=>b.complete);
...
export function propertyAdjustments(subject:ComparisonProperty,comparable:ComparisonProperty,taxYear=2026) {
  ...
  const lines:AdjustmentLine[]=[
    ...
    {factor:"Non-living details",amount:calculation.amounts["Non-living details"],explanation:"Your primary building’s non-living detail costs minus the comparable’s, before neighborhood multipliers. Garages, porches, pools and other features attached to that improvement are counted here, once.",inputs:[input("Your non-living details",si.nonliving),input("Comparable non-living details",ci.nonliving),input("Your recorded non-living features",s.main?.features.filter(f=>!["1ST","2ND","3RD"].includes(f.code)).map(f=>f.description||f.code).join(", ")||"None recorded"),input("Comparable recorded non-living features",c.main?.features.filter(f=>!["1ST","2ND","3RD"].includes(f.code)).map(f=>f.description||f.code).join(", ")||"None recorded")]},
