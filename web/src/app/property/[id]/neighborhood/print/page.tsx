...
export default async function NeighborhoodPrintPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<EvidenceQuery>}) {
 ...
 return <main id="main-content" className="neighborhood-print-preview"><div className="print-toolbar"><Link href={backHref}>← Back to neighborhood analysis</Link><PrintButton/></div><article className="neighborhood-page print-report"><style>{`@page {@bottom-left {content: ${JSON.stringify(`Property ${id} | ${analysis.current.tax_year} ${analysis.current.roll_stage}`)};}}`}</style><header className="print-header"><BrandLogo/><p>Neighborhood report · {analysis.current.tax_year} {analysis.current.roll_stage}</p></header><h1>Your neighborhood, in context.</h1><dl className="print-property-facts"><div><dt>Property</dt><dd>{d.subject.address}<br/>Property {id}</dd></div><div><dt>Home context</dt><dd>{d.subject.living_area?.toLocaleString('en-US')??'Unreported'} sq ft · Class {d.subject.class_code??'not reported'}</dd></div><div><dt>Subdivision on record</dt><dd>{d.subdivision??'Not reported'}</dd></div><div><dt>Appraisal District group</dt><dd>{d.neighborhood}</dd></div><div><dt>Current eligible group population</dt><dd>{d.homes.length.toLocaleString('en-US')} single-family homes</dd></div><div><dt>Current source</dt><dd>{analysis.current.tax_year} {analysis.current.roll_stage} · {dateLabel(analysis.current.export_date)}</dd></div></dl><p className="print-group-note">The Appraisal District group is a market-area code used for analysis. It can differ from the subdivision recorded for this home.</p><NeighborhoodReport data={d} analysis={analysis}/>{error?<p role="alert">{error} Return to the neighborhood page to choose a valid evidence window.</p>:<ActivityShortlistPrint data={activity} window={window} query={query}/>}<NeighborhoodReportSources analysis={analysis}/><footer className="print-report-footer"><p>Generated {new Intl.DateTimeFormat('en-US',{dateStyle:'medium',timeZone:'America/Chicago'}).format(new Date())} · Method {NEIGHBORHOOD_METHOD_VERSION}<br/>Live analysis: <a href={`${SITE_URL}${backHref}`}>{`${SITE_URL}/property/${id}/neighborhood`}</a></p></footer></article></main>;
}
