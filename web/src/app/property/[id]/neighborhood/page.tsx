...
export default async function NeighborhoodPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{activityYear?:string}>}) {
 ...
 const query=await searchParams,yr=typeof query.activityYear==='string'&&/^\d{4}$/.test(query.activityYear)?Number(query.activityYear):null;
 const [result,activity]=await Promise.all([getNeighborhoodAnalysis(id,season),getPropertyActivity(id,yr!==null&&yr>=1900&&yr<=2200?yr:null)]);
