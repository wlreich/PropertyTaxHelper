...
test('market factors, matched median, unavailable preliminary baseline and sources',async({page},info)=>{
 ...
 await expect(panel).toContainText('ParcelSavvy estimates how the Appraisal District’s market-area multiplier affected the building value. Land and other valuation changes are separate.');
 ...
 await expect(panel).toContainText('It is not a complete appraisal, tax saving, or proof the appraisal is wrong.');
 ...
 await expect(panel).toContainText('reproduce the recorded preliminary improvement value within $1');
 ...
 await expect(panel).toContainText('ParcelSavvy estimates how the Appraisal District’s market-area multiplier affected building values in this group. Land and other valuation changes are separate.');
 await expect(panel).toContainText('Estimated median effect');
 await expect(panel).toContainText('2 matched homes');
 await expect(panel).toContainText('These 2026 dollar estimates change only the multiplier from 2025 1.46× to 2026 1.78×');
 await expect(panel).toContainText('They are not a complete appraisal, a whole-neighborhood total, tax savings, or proof the appraisal is wrong.');
