...
test('PAR-25 compact ledger, year dialog, provenance and focus restoration',async({page},info)=>{
  ...
  const progression=page.getByRole('region',{name:'Valuation progression'});
  ...
  await expect(progression.getByRole('heading',{name:'Valuation progression'})).toBeVisible();
  expect((await progression.boundingBox())!.y).toBeLessThan((await page.locator('#market-adjustment').boundingBox())!.y);
  ...
  await expect(progression.locator('.annual-chart-year')).toHaveCount(2);
  await expect(progression.locator('.annual-chart-caption')).toContainText('explicitly eligible preliminary records');
  await expect(progression.locator('.annual-chart-caption')).toContainText('completed certified or supplemental records');
  await expect(progression.locator('.annual-chart-caption')).toContainText('does not establish the cause');
  const stages2025=progression.locator('[data-chart-year="2025"] [data-chart-stage]');
  ...
  const stages2026=progression.locator('[data-chart-year="2026"] [data-chart-stage]');
  ...
  for(const label of ['Proposed market value','Final market value','Assessed value']) await expect(progression.locator('.annual-chart')).toContainText(label);
  await expect(progression).toContainText('widened from $112,899 in 2025 to $197,959 in 2026');
  ...
  if(info.project.use.viewport!.width===1440) {
    ...
    expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
    await progression.screenshot({path:info.outputPath('par57-mobile-390.png')});
...
test('PAR-25 one, five and six years stay bounded through navigation and dialogs',async({page},info)=>{
  for(const [id,count,earlier] of [['999111',1,false],['999115',5,false],['999116',5,true]] as const) {
    ...
    if(earlier) {
      ...
      const progression=page.locator('.annual-progression');
      await expect(progression).toContainText('Latest five years shown');
      await expect(progression.locator('[data-chart-year="2021"]')).toHaveCount(0);
      await expect(progression.locator('.annual-chart-year')).toHaveCount(5);
...
test('PAR-25 missing, nonconsecutive, preliminary-only and excluded-baseline histories',async({page})=>{
  ...
  const progression=page.locator('.annual-progression');
  const futureStages=progression.locator('[data-chart-year="2027"] [data-chart-stage]');
  ...
  await expect(progression.locator('[data-chart-year="2025"] [data-chart-stage="proposed"]')).toHaveAccessibleName('2025 Proposed market value: Not available');
