...
test('activity filters, keyboard selection, shortlist and responsive layout',async({page},info)=>{
 ...
 await section.getByRole('combobox',{name:'Sort activity'}).selectOption('newest');await section.getByRole('button',{name:'Show latest five'}).click();await section.getByRole('checkbox').first().check();
