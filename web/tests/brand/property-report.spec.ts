...
test('PAR-28 complete Letter reports preserve inventory, history and page furniture',async({page},info)=>{
 ...
 const pageCounts={'synthetic-reference':8,'dense-inventory':10,'long-history':9,'limited-data':2} as const;
 for(const [id,name,features,years] of [['999283','synthetic-reference',11,2],['999280','dense-inventory',26,2],['999281','long-history',0,26],['999282','limited-data',1,1]] as const){
 ...
 await expect(page.locator('[data-report-record^="history-"]')).toHaveCount(years);
  ...
  if(id==='999281'){await expect(page.locator('[data-report-trend-year]')).toHaveCount(5);await expect(page.locator('[data-report-trend-year="2026"] [data-report-trend-stage]')).toHaveCount(3);await expect(page.locator('[data-report-trend-year="2026"] [data-report-trend-stage="proposed"]')).toHaveAccessibleName('2026 Proposed market value: $889,000');}
  ...
  if(id==='999282'){await expect(page.locator('[data-report-trend-year="2026"] [data-report-trend-stage="proposed"]')).toHaveAccessibleName('2026 Proposed market value: Unavailable');await expect(page.locator('[data-report-trend-year="2026"] [data-report-trend-stage="final"]')).toHaveAccessibleName('2026 Final market value: Pending');}
