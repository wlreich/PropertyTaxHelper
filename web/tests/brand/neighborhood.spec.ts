...
test('neighborhood responsive layout and zoom',async({page},info)=>{
 ...
 const agents=page.locator('.neighborhood-agent-activity'),agentPanels=agents.locator('.neighborhood-agent-panel');await expect(agentPanels).toHaveCount(2);await expect(agentPanels.nth(0)).toContainText('2026 certified appraisal');await expect(agentPanels.nth(0)).toContainText('export Jul 18, 2026');await expect(agentPanels.nth(1)).toContainText('2025 certified appraisal');await expect(agentPanels.nth(1)).toContainText('Other agents (1)');await expect(agents.getByRole('img').first()).toHaveAttribute('aria-label',/properties on a shared scale with a maximum of/);
