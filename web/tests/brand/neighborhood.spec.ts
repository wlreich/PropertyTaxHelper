...
test('neighborhood annual story, controls, canonical links and print parity',async({page},info)=>{
 ...
 await expect(capProgression).toContainText(`These stages follow the same ${started![1]} homes that started above the cap.`);await expect(expanded).toContainText(/2025: \d+ of \d+ paired homes reduced/);await expect(expanded).not.toContainText('0 / 0');
...
test('neighborhood responsive layout and zoom',async({page},info)=>{
 ...
 const more=page.locator('summary').filter({hasText:'More annual outcomes and comparisons'});await more.click();const cap=more.locator('..').locator('.neighborhood-cap-progression');await expect(cap).toBeVisible();await expect(more.locator('..').getByRole('region',{name:'2026 protest activity comparison'})).toBeVisible();
