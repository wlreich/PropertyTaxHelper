...
import { checkDatabaseHealth } from "../src/lib/supabase/health.ts";
...
test("missing or privileged keys never initiate a database request", async () => {
  for (const config of [{}, { ...configuration, SUPABASE_PUBLISHABLE_KEY: "sb_secret_fixture" }]) {
...
test("success requires the read-only RPC response, using the publishable key", async () => {
  const result = await checkDatabaseHealth(configuration, async (input, init) => {
    assert.equal(new URL(input).pathname, "/rest/v1/rpc/database_health");
    assert.equal(init.method, "GET");
    assert.equal(new Headers(init.headers).get("apikey"), configuration.SUPABASE_PUBLISHABLE_KEY);
...
test("an HTTP success with an unexpected result is not database health", async () => {
  for (const value of [null, 0, "1", { status: "ok" }]) {
    assert.equal(await checkDatabaseHealth(configuration, async () => Response.json(value)), "unavailable");
  }
});
...
test("network failures and aborted requests do not escape as raw errors", async () => {
  for (const error of [new Error("Sensitive network detail"), new DOMException("timeout", "TimeoutError")]) {
    const result = await checkDatabaseHealth(configuration, async () => { throw error; });
    assert.equal(result, "unavailable");
  }
});
