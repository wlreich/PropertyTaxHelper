...
test('migration gate distinguishes missing versions from known version drift', async () => {
  ...
  assert.match(missing.failures[0], /missing migration 20260926183000/);
  const drift = await checkMigrationParity('20260926173000\n');
  assert.match(drift.failures[0], /version drift.*20260926173000.*20260926183000/);
  assert.deepEqual((await checkMigrationParity('20260926183000\n')).failures, []);
});
