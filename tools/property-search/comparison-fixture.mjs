export async function seedComparisons(db) {
 ...
 for(const [id,address,area,year,value] of [['120','120 CYPRESS ST',2000,2014,400000],['121','121 CYPRESS ST',2010,2015,460000],['122','122 CYPRESS ST',2080,2016,480000],['123','123 PINE ST',2400,2000,null],['124','124 LONG SYNTHETIC CYPRESS RIDGE TERRACE NORTHWEST UNIT 100',2000,2014,450000]]) {
  ...
  await db.query('insert into public.property_snapshot_profiles values($1,$2,$3,$4)',[anchor,anchor,id,{...snapshot(area,year,value),...(id==='124'?{neighborhood:'OTHER'}:{})}]);
 }
 ...
 for(const source of [anchor,old]) {
  for(const [member,type] of [['cost-land.txt','Property'],['comparison-long-address-land.txt','Property'],['cost-buildings.txt','Improvement'],['cost-details.txt','ImprovementDetail']])
  ...
  let row=0;
  for(const [id,area,year] of [['100',2000,2014],['120',2000,2014],['121',2010,2015],['122',2080,2016],['123',2400,2000],['124',2000,2014],['102',2000,2014],['103',2000,2014]]) {
   // The long-address record keeps its own land source when neighborhood
   // fixtures replace the other comparison-only Property rows.
   const values=[[id==='124'?'comparison-long-address-land.txt':'cost-land.txt',{land_hstd_val:'100000',land_non_hstd_val:'0',ag_market:'0',imprv_state_cd:'A1',land_state_cd:'A1',abs_subdv_cd:'GRAND02'}],['cost-buildings.txt',{imprv_id:'1',imprv_val:'350000',imprv_type_cd:'01',imprv_state_cd:'A1',owner_name:'PRIVATE MUST NOT LEAK'}],['cost-details.txt',{imprv_id:'1',imprv_det_id:'1',imprv_det_val:'350000',imprv_det_area:String(area),imprv_det_class_cd:'R3',yr_built:String(year),depreciation_yr:String(year),imprv_det_type_cd:'1ST',imprv_det_type_desc:'Main area'}]];
 ...
 await db.query(`update tcad_ingest.records set fields=fields||'{"imprv_val":"1000000"}'::jsonb where dataset_id=$1 and member_name='cost-buildings.txt' and prop_id='123'`,[anchor]);
 await db.query(`insert into tcad_ingest.records(dataset_id,member_name,row_number,prop_id,prop_val_yr,fields) values($1,'cost-buildings.txt',999,'123','2026',$2)`,[anchor,{imprv_id:'second',imprv_val:'627149',imprv_type_cd:'01',imprv_state_cd:'A1'}]);
 return {anchor,old};
}
