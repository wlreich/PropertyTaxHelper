// Synthetic data only. Run: node benchmark-numbered-search.mjs [row-count]
// Compare the old/new public RPCs in the same database, with RLS and generic plans.
...
try {
 ...
 for(const name of ['20260913154859_address_search_v2','20260913155642_address_search_candidates','20260914192337_property_search_suggestions']) await db.exec(await migration(name));
 ...
 const cases=[['suggest house','select public.suggest_property_parcels($1,8,false) result','1104'],
  ['suggest street','select public.suggest_property_parcels($1,8,false) result','SAMPLE'],
 ...
 await db.exec(await migration('20260918223337_indexed_numbered_property_search'));
 await db.exec('analyze public.property_search_documents; set role anon');
 const after=await measure();
 for(let i=0;i<before.length;i++) assert.deepEqual(after[i].result,before[i].result);
 ...
 and search_text collate "C">='1104 ' and search_text collate "C"<'1104!'`)).rows;
