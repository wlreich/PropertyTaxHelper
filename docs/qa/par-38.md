...
Production database preparation: migration `20260921180503_activity_match_inputs.sql` is applied with the matching Supabase history version. A read-only anonymous-role probe returned the requested published matching inputs; the wrapper is security-invoker and anonymous raw-ingestion access remains false. UI deployment awaits final CI and merge.
