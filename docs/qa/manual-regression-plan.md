...
This file preserves the manual/agent test specification and templates from the initial QA package. The installed Playwright jobs, schedule and exact automated coverage are documented in `home-search-regression.md`; do not interpret the full manual catalogue as automated coverage.
