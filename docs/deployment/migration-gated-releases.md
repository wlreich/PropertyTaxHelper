...
3. Merge only after review and required checks pass. Vercel's existing Git integration starts the deployment. On production builds, `web/scripts/check-release-contract.mjs` makes a read-only anon RPC request for property 736302 using the existing server environment and runs the page's own parser on that response. Missing config, HTTP errors, invalid stage/source/population, missing `agent_assignments`, or malformed nested entries fail the build before promotion.
