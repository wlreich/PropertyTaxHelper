...
`web/vercel.json` keeps automatic Git deployments enabled. Its build command runs a read-only published-database contract check before a production build; a failed check prevents the new deployment from replacing the current one. The check uses the website's existing server-side Supabase URL and publishable key. Apply and verify reviewed migrations before merging code that requires them; the migration-parity tool in `tools/release-contract` distinguishes missing or drifted recorded versions. A configuration file does not itself prove the account integration, production branch, deployment, or domain assignment is correct; verify the resulting deployment when access is available.
