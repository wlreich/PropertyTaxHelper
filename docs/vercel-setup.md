...
`web/vercel.json` disables automatic Git deployments so a merge cannot bypass required database checks. The **Migration-gated web release** workflow runs automatically for relevant pull requests, verifies migration parity and the published database contract, and then invokes Vercel for nonproduction. After merge, run nonproduction again on the resulting `main` SHA; production requires that exact commit's successful main nonproduction evidence. A configuration file does not itself prove the account integration, environment credentials, production branch, deployment, or domain assignment is correct; verify the resulting deployment when access is available.
