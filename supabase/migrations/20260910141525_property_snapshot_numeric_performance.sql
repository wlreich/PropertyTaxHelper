-- Avoid per-field SQL executor overhead while retaining strict numeric parsing.
create function tcad_ingest.profile_number(value text) returns numeric
...
-- Decompress each source row once; retain only the fields used by publication.
...
begin
 ...
 case when f.record_type='Property' then (select jsonb_object_agg(key,value) from jsonb_each(r.fields) where key in
 ...
 or (right(key,7)='_exempt' and value='"T"'::jsonb))
 when f.record_type='PropertyEntity' then (select jsonb_object_agg(key,value) from jsonb_each(r.fields) where key in ('entity_cd','entity_name','taxable_val')
 or (right(key,4)='_amt' and key not in ('hs_local_amt','hs_state_amt') and (value#>>'{}') ~ '^[0-9]+([.][0-9]+)?$' and (value#>>'{}') !~ '^[0.]+$'))
