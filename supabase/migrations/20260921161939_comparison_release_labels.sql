-- PAR-30: carry existing published baseline eligibility into comparison release metadata.
-- Selection, costs, access rules and candidate queries are unchanged.
...
begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object('dataset_id',dataset_id,'tax_year',snapshot->'tax_year','roll_stage',snapshot->'roll_stage',
  'export_date',snapshot->'export_date','preliminary_baseline_eligible',snapshot->'preliminary_baseline_eligible') order by snapshot->>'tax_year' desc,snapshot->>'export_date' desc,dataset_id),'[]') into releases
 ...
 source_meta:=jsonb_build_object('dataset_id',source_id,'tax_year',subject_snapshot->'tax_year','roll_stage',subject_snapshot->'roll_stage','export_date',subject_snapshot->'export_date','preliminary_baseline_eligible',subject_snapshot->'preliminary_baseline_eligible');
