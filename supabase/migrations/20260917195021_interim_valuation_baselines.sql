-- Keep publisher roll labels intact while excluding reviewed interim values from notice comparisons.
alter table tcad_ingest.datasets add column preliminary_baseline_excluded boolean not null default false;
alter table tcad_ingest.datasets add column valuation_note text check(length(valuation_note)<=1000);
comment on column tcad_ingest.datasets.preliminary_baseline_excluded is
 'Reviewed exclusion from original-notice comparisons; false preserves existing eligibility, not proof of a notice.';
update tcad_ingest.datasets set preliminary_baseline_excluded=true,
...
begin
 ...
 select anchor,p_dataset,p.id,jsonb_build_object(
  ...
  'preliminary_baseline_eligible',not d.preliminary_baseline_excluded,'valuation_note',d.valuation_note,
...
begin
 if p_pre is null or p_pre->>'preliminary_baseline_eligible'='false' then reason:='missing_preliminary';
 ...
 if p_pre->>'preliminary_baseline_eligible' is distinct from 'false'
  and p_old_pre->>'preliminary_baseline_eligible' is distinct from 'false'
  and p_pre->>'neighborhood'=p_code and p_old_pre->>'neighborhood'=p_code
...
create or replace function public.market_adjustment_analysis(p_anchor uuid,p_source uuid,p_ids text[],p_subject text)
