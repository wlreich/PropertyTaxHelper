-- Curated, non-personal activity. Raw source tables remain private.
create table public.property_activity_releases (
 anchor_dataset_id uuid not null, activity_year integer not null,
 appraisal_export_date date not null, sales_export_date date not null,
 import_id uuid not null, published_at timestamptz not null default now(),
 primary key(anchor_dataset_id,activity_year)
);
create table public.property_activity (
 anchor_dataset_id uuid not null, activity_year integer not null,
 property_id text not null, event_key text not null,
 deed_date date, sale_date date, filed_date date, instrument text,
 deed_type text, sale_type text, sale_source text,
 status text not null check(status in('deed_change','sale_recorded')),
 price numeric(18,2), price_status text not null check(price_status in('reported','not_reported','multi_property')),
 match_method text check(match_method in('deed_id','instrument','unique_date')),
 deed_source text check(deed_source in('appraisal','supplemental')),
 primary key(anchor_dataset_id,activity_year,property_id,event_key),
 foreign key(anchor_dataset_id,activity_year) references public.property_activity_releases,
 check((price_status='reported')=(price is not null)), check(price>0),
 check(deed_date is not null or sale_date is not null)
);
create index property_activity_lookup on public.property_activity(anchor_dataset_id,property_id,activity_year);
alter table public.property_activity_releases enable row level security;
alter table public.property_activity enable row level security;
revoke all on public.property_activity_releases,public.property_activity from public,anon,authenticated;
grant select on public.property_activity_releases,public.property_activity to anon,authenticated;
create policy activity_release_read on public.property_activity_releases for select to anon,authenticated
 using(anchor_dataset_id=(select dataset_id from public.property_search_state where singleton));
create policy activity_read on public.property_activity for select to anon,authenticated
 using(anchor_dataset_id=(select dataset_id from public.property_search_state where singleton)
 and exists(select 1 from public.property_search_documents d where d.dataset_id=anchor_dataset_id and d.property_id=property_activity.property_id
 and not d.shared_ownership and not d.values_under_review and not d.is_parkland));

create function tcad_ingest.activity_date(p_text text) returns date
 language plpgsql immutable set search_path='' as $$
declare d date;
begin
 if p_text !~ '^\d{2}-\d{2}-\d{4}$' then return null; end if;
 d:=make_date(substr(p_text,7,4)::int,substr(p_text,1,2)::int,substr(p_text,4,2)::int);
 return d;
exception when datetime_field_overflow or invalid_datetime_format then return null;
end $$;
revoke all on function tcad_ingest.activity_date(text) from public,anon,authenticated;

create function tcad_ingest.publish_property_activity(p_year integer) returns jsonb
...
begin
 ...
 select *,min(priority) over(partition by property_id,event_id) best from candidates
 ), unique_links as (
 select *,count(*) over(partition by property_id,event_key) deeds,count(*) over(partition by property_id,event_id) sales from best where priority=best
...
create function public.property_neighborhood_activity(p_id text,p_year integer default null) returns jsonb
...
begin
 ...
 left join lateral(select t from jsonb_array_elements(types) t where t->>'property_id'=a.property_id) t on true
