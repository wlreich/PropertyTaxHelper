-- Dataset eligibility is separate from a single property's source conflict.
-- This private-schema helper exposes only a boolean for a visible published
-- subject/release; it grants no access to ingestion tables or source records.
create function parcel_comparison.preliminary_release_eligible(p_anchor uuid,p_source uuid,p_property text)
returns boolean language sql stable security definer set search_path='' set statement_timeout='2s' as $$
 select exists(
  select 1 from public.property_search_state a
  join public.property_snapshot_profiles s on s.anchor_dataset_id=a.dataset_id
  join public.property_search_documents v on v.dataset_id=a.dataset_id and v.property_id=s.property_id
  join tcad_ingest.datasets d on d.id=s.dataset_id
  where a.singleton and a.dataset_id=p_anchor and s.dataset_id=p_source and s.property_id=p_property
   and not v.shared_ownership and not v.values_under_review and not v.is_parkland
   and d.status='ready' and not d.preliminary_baseline_excluded
 )
$$;
revoke all on function parcel_comparison.preliminary_release_eligible(uuid,uuid,text) from public;
grant execute on function parcel_comparison.preliminary_release_eligible(uuid,uuid,text) to anon,authenticated;
