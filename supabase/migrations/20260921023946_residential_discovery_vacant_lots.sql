...
create temporary table vacant_lot_classifications on commit drop as
 ...
 select p.dataset_id,ltrim(r.prop_id,'0') property_id,
 ...
 left join parcel_admin.preparations prep on prep.id=p.dataset_id
 join tcad_ingest.files f on f.dataset_id=coalesce(prep.source_dataset,p.dataset_id) and f.record_type='Property'
 ...
 group by p.dataset_id,ltrim(r.prop_id,'0');
...
-- Routine admin preparations use separate release identities.
create or replace function tcad_ingest.prepare_property_search(p_dataset_id uuid,p_anchor uuid) returns jsonb
language plpgsql security invoker set search_path='' as $$
declare d record; published_count bigint;
begin
 ...
 get diagnostics published_count = row_count;
 if published_count=0 then raise exception 'No eligible properties; publication rolled back'; end if;
 -- Activation remains separate and gated on completed preparation.
 analyze public.property_search_documents;
