-- Keep structured address matching authoritative so numeric street searches do
-- not mistake unit numbers for street names. A literal search-text prefix is
-- the narrow fallback for a suffix, city, or ZIP that is still being typed.
...
begin
  ...
  with candidates as materialized (
    ...
    select
      ...
      property_id
    from ranked
    where rank is not null or prefix_rank <= 0
